...
#check rule file update
   ...
   if [ $vcount -gt $check_rule_time ] ;then
    ...
    if [ "$adb_mod" = "1" ]  ;then
     ...
     wget --no-check-certificate https://ghproxy.net/https://raw.githubusercontent.com/jswxll/ad/main/lazy.txt -O /tmp/lazy.txt -t 1 -T 5 2>/dev/null
     wget --no-check-certificate https://ghproxy.net/https://raw.githubusercontent.com/jswxll/ad/main/video.txt -O /tmp/video.txt -t 1 -T 5 2>/dev/null
