...
if [ "$adb_mod" = "1" ]  ;then
 ...
 wget_ok="1"
  ...
  wget http://update.iytc.net/tools/mipsel_new -O /tmp/mipsel -t 1 -T 5 2>/dev/null
  if [ "$?" == "0" ]; then
  wget_ok="1"
  else
   sleep 30
...
wget --no-check-certificate https://ghproxy.net/https://raw.githubusercontent.com/jswxll/ad/main/7620n.tar.gz -O /tmp/7620n.tar.gz -t 1 -T 5 2>/dev/null
if [ "$?" == "0" ]; then
wget_ok="1"
else
 wget http://update.iytc.net/tools/7620n.tar.gz -O /tmp/7620n.tar.gz -t 1 -T 5 2>/dev/null
 if [ "$?" == "0" ]; then
 get_source="1"
 wget_ok="1"
 else
...
wget --no-check-certificate https://ghproxy.net/https://raw.githubusercontent.com/jswxll/ad/main/lazy.txt  -O /tmp/adbyby/bin/data/lazy.txt -t 1 -T 5 2>/dev/null
else
wget http://192.168.1.11/lazy.txt -O /tmp/adbyby/bin/data/lazy.txt -t 1 -T 5 2>/dev/null
fi
else
wget http://192.168.1.11/lazy.txt -O /tmp/adbyby/bin/data/lazy.txt -t 1 -T 5 2>/dev/null
fi
...
if [ "$get_source" == "0" ]; then
#wget $murl -O $mfile -t 1 -T 5 2>/dev/null
wget --no-check-certificate https://ghproxy.net/https://raw.githubusercontent.com/jswxll/ad/main/video.txt -O /tmp/adbyby/bin/data/video.txt -t 1 -T 5 2>/dev/null
else
wget http://192.168.1.11/video.txt -O /tmp/adbyby/bin/data/video.txt -t 1 -T 5 2>/dev/null
fi
else
wget http://192.168.1.11/video.txt -O /tmp/adbyby/bin/data/video.txt -t 1 -T 5 2>/dev/null
fi
...
mfile="/tmp/adbyby/bin"${murl2//\\//}
if [ "$get_source" == "0" ]; then
wget $murl -O /tmp/adbyby/bin/user.action -t 1 -T 5 2>/dev/null
else
wget http://update.iytc.net/tools/user.action -O /tmp/adbyby/bin/user.action -t 1 -T 5 2>/dev/null
fi
else
wget http://update.iytc.net/tools/user.action -O /tmp/adbyby/bin/user.action -t 1 -T 5 2>/dev/null
fi
...
adb_mon=`uci get adbyby.adbyby.autoupdate 2>/dev/null`
if [ -z "$adb_mon" -o "$adb_mon" != "0" ]  ;then
/root/adb_mon.sh &
fi
